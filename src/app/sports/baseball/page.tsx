...
export default function BaseballPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(`/api/sports/baseball?endpoint=${activeTab}`)
        if (!response.ok) {
  ...
  const renderGameTime = (game: Game) => {
    if (game.Status === 'InProgress') {
      return `${game.InningHalf === 'T' ? 'Top' : 'Bottom'} ${game.Inning}`
    }
        ...
        <TableBody>
          {standings.map(team => (
            <TableRow key={team.TeamID}>
              ...
              <TableCell className="text-right">{team.Percentage.toFixed(3)}</TableCell>
        ...
        <CardContent className="pt-6">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div className="border rounded-md p-3">
              <div className="text-sm text-muted-foreground mb-1">Run Line</div>
  ...
  return (
    <div className="container py-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">Baseball</h1>
        <p className="text-muted-foreground">
          Live scores, standings, and betting odds for baseball games
        </p>
      </div>
