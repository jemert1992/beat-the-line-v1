...
export default function BasketballPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        const response = await fetch(`/api/sports/basketball?endpoint=${activeTab}`)
        if (!response.ok) {
  ...
  const renderGameTime = (game: Game) => {
    if (game.Status === 'InProgress') {
      return `Q${game.Quarter} ${game.TimeRemainingMinutes}:${game.TimeRemainingSeconds?.toString().padStart(2, '0')}`
    }
  ...
  return (
    <div className="container py-6">
      <div className="mb-6">
        <h1 className="text-3xl font-bold mb-2">Basketball</h1>
        <p className="text-muted-foreground">
          Live scores, standings, and betting odds for basketball games
        </p>
      </div>
